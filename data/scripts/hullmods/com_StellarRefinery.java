...
public class com_StellarRefinery extends BaseHullMod {

    private static final float FUEL_PER_DAY = 12f;
    private static final float FUEL_PER_DAY_SMOD = 16f;

    // Must match hull_mods.csv id
    private static final String HULLMOD_ID = "com_stellar_refinery";
    ...
    public void advanceInCampaign(FleetMemberAPI member, float amount) {
        ...
        // Only produce while in hyperspace:
        if (!fleet.isInHyperspace()) return;

        CargoAPI cargo = fleet.getCargo();
        if (cargo == null) return;

        // STOP production if fuel storage is overfull (ABOVE capacity, not AT capacity)
        ...
        float perDay = isSMod ? FUEL_PER_DAY_SMOD : FUEL_PER_DAY;
